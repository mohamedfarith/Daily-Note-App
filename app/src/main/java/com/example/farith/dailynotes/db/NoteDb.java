...
public class NoteDb extends SQLiteOpenHelper {
    ...
    public void insertData(SQLiteDatabase db, String data, String time, String notificationId, String reminderTime) {
        db = getWritableDatabase();
    ...
    public ArrayList<NotesDatabaseList> readValues(SQLiteDatabase db) {
        ...
        if (cursor.equals(null)) {
            ...
            while (cursor.moveToNext()) {
                ...
                String notificationId = cursor.getString(cursor.getColumnIndex(NoteClass.NOTIFICATION_ID));
                String reminderTime = cursor.getString(cursor.getColumnIndex(NoteClass.REMINDER_TIME));
                NotesDatabaseList list = new NotesDatabaseList();
                list.setNotes(notes);
                list.setDate(date);
                list.setNotificationID(notificationId);
                list.setReminder(reminderTime);
                notesDatabaseList.add(list);
                Log.d(TAG, "readValues: from the database: edited date from db " + list.date + " edited notes from db " + list.notes + " notification id from db " + list.notificationID + " Reminder time from db" + reminderTime);
            }
        }
    ...
    public void updateDb(SQLiteDatabase db, String note, String previousTime, String currentTime, String notificationId, String reminderTime) {
